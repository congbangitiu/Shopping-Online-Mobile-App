...
const ConfirmationScreen = () => {
    ...
    const handlePlaceOrder = async () => {
        try {
            ...
            const response = await axios.post('http://192.168.1.8:3000/orders', orderData);
    ...
    const pay = async () => {
        try {
            const options = {
                ...
                currency: 'INR',
                name: 'Amazon',
                key: 'rzp_test_BVFdhYRHnd00Qf', // Thay YOUR_RAZORPAY_KEY bằng khóa Razorpay của bạn
                amount: total * 100,
                prefill: {
            ...
            RazorpayCheckout.open(options)
                .then((data) => {
                    ...
                    // Tiếp tục với xử lý đặt hàng và điều hướng
                    const orderData = {
                        userId: userId,
                        cartItems: cart,
                        totalPrice: total,
                    ...
                    axios
                        .post('http://192.168.1.8:3000/orders', orderData)
                        .then((response) => {
                            if (response.status === 200) {
                                navigation.navigate('Order');
                                dispatch(cleanCart());
            ...
            <View style={{ flex: 1, paddingHorizontal: 20, paddingTop: 40 }}>
                ...
                {currentStep === 3 && selectedOption === 'cash' && (
                    <View style={{ marginHorizontal: 20 }}>
                        ...
                        >
                            <Text style={{ fontWeight: '600' }}>Shipping to {selectedAddress?.name}</Text>
                        ...
                        >
                            <Text style={{ fontSize: 16, color: 'gray', fontWeight: '600' }}>Pay With</Text>
